...
const toUint8Array = (data) => {
    if (typeof data === "string") {
        return parseUtils.fromUtf8(data);
    }
...
class Uint8ArrayBlobAdapter extends Uint8Array {
    static fromString(source, encoding = "utf-8") {
        if (typeof source === "string") {
            if (encoding === "base64") {
                return Uint8ArrayBlobAdapter.mutate(parseUtils.fromBase64(source));
            }
            return Uint8ArrayBlobAdapter.mutate(parseUtils.fromUtf8(source));
        }
    ...
    transformToString(encoding = "utf-8") {
        if (encoding === "base64") {
            return parseUtils.toBase64(this);
        }
        return parseUtils.toUtf8(this);
    }
}
...
const getChecksumConfiguration = (runtimeConfig) => {
    ...
    for (const id in index.AlgorithmId) {
        const algorithmId = index.AlgorithmId[id];
...
const getDateHeader = (response) => index.HttpResponse.isInstance(response) ? response.headers?.date ?? response.headers?.Date : undefined;
...
class AwsSdkSigV4Signer {
    async sign(httpRequest, identity, signingProperties) {
        if (!index.HttpRequest.isInstance(httpRequest)) {
...
const resolveAwsSdkSigV4Config = (config) => {
    ...
    Object.defineProperty(config, "credentials", {
        set(credentials) {
            ...
            if (isUserSupplied && !boundProvider.attributed) {
                resolvedCredentials = async (options) => boundProvider(options).then((creds) => index.setCredentialFeature(creds, "CREDENTIALS_CODE", "e"));
    ...
    if (config.signer) {
        ...
        signer = () => normalizeProvider(config.region)()
            ...
            const SignerCtor = config.signerConstructor || index.SignatureV4;
...
const recursionDetectionMiddleware = () => (next) => async (args) => {
    ...
    const traceIdFromInvokeStore = index.InvokeStore.getXRayTraceId();
...
const DEFAULT_PORTS = {
    [index.EndpointURLScheme.HTTP]: 80,
    [index.EndpointURLScheme.HTTPS]: 443,
};
const parseURL = (value) => {
    ...
    if (!Object.values(index.EndpointURLScheme).includes(scheme)) {
        return null;
    }
...
const collectBodyString = (streamBody, context) => collectBody(streamBody, context).then((body) => (context?.utf8Encoder ?? parseUtils.toUtf8)(body));
...
const getEndpointUrlConfig = (serviceId) => ({
    ...
    configFileSelector: (profile, config) => {
        if (config && profile.services) {
            const servicesSection = config[["services", profile.services].join(loadSharedConfigFiles.CONFIG_PREFIX_SEPARATOR)];
            if (servicesSection) {
                const servicePrefixParts = serviceId.split(" ").map((w) => w.toLowerCase());
                const endpointUrl = servicesSection[[servicePrefixParts.join("_"), CONFIG_ENDPOINT_URL].join(loadSharedConfigFiles.CONFIG_PREFIX_SEPARATOR)];
                if (endpointUrl)
                    return endpointUrl;
            }
        }
...
const getEndpointFromConfig = async (serviceId) => index$1.loadConfig(getEndpointUrlConfig(serviceId ?? ""))();

const toEndpointV1 = (endpoint) => {
    if (typeof endpoint === "object") {
        if ("url" in endpoint) {
            return index$1.parseUrl(endpoint.url);
        }
        return endpoint;
    }
    return index$1.parseUrl(endpoint);
};
...
const resolveEndpointConfig = (input) => {
    ...
    const customEndpointProvider = endpoint != null ? async () => toEndpointV1(await index.normalizeProvider(endpoint)()) : undefined;
    const isCustomEndpoint = !!endpoint;
    const resolvedConfig = Object.assign(input, {
        endpoint: customEndpointProvider,
        tls,
        isCustomEndpoint,
        useDualstackEndpoint: index.normalizeProvider(useDualstackEndpoint ?? false),
        useFipsEndpoint: index.normalizeProvider(useFipsEndpoint ?? false),
    });
...
const resolveRetryConfig = (input) => {
    ...
    const maxAttempts = index.normalizeProvider(_maxAttempts ?? DEFAULT_MAX_ATTEMPTS);
    return Object.assign(input, {
        maxAttempts,
        retryStrategy: async () => {
            if (retryStrategy) {
            ...
            const retryMode = await index.normalizeProvider(_retryMode)();
...
const isStreamingPayload = (request) => request?.body instanceof require$$0.Readable ||
...
const retryMiddleware = (options) => (next, context) => async (args) => {
    ...
    if (isRetryStrategyV2(retryStrategy)) {
        ...
        const { request } = args;
        const isRequest = index.HttpRequest.isInstance(request);
        if (isRequest) {
            request.headers[INVOCATION_ID_HEADER] = index.v4();
        }
        while (true) {
            try {
                if (isRequest) {
...
const createDefaultUserAgentProvider = ({ serviceId, clientVersion }) => {
    return async (config) => {
        const sections = [
            ...
            ["ua", "2.1"],
            [`os/${require$$0$1.platform()}`, require$$0$1.release()],
            ["lang/js"],
...
function castSourceData(toCast, encoding) {
    if (require$$0$2.Buffer.isBuffer(toCast)) {
        return toCast;
    }
    if (typeof toCast === "string") {
        return parseUtils.fromString(toCast, encoding);
    }
    if (ArrayBuffer.isView(toCast)) {
        return parseUtils.fromArrayBuffer(toCast.buffer, toCast.byteOffset, toCast.byteLength);
    }
    return parseUtils.fromArrayBuffer(toCast);
}
...
const resolveDefaultsModeConfig = ({ region = index$1.loadConfig(NODE_REGION_CONFIG_OPTIONS), defaultsMode = index$1.loadConfig(NODE_DEFAULTS_MODE_CONFIG_OPTIONS), } = {}) => index$1.memoize(async () => {
...
const inferPhysicalRegion = async () => {
    ...
    if (!process.env[ENV_IMDS_DISABLED]) {
        try {
            const { getInstanceMetadataEndpoint, httpRequest } = await Promise.resolve().then(function () { return require('./index-DE1JYBBX.js'); });
...
//# sourceMappingURL=index-CoEzlyBk.js.map
